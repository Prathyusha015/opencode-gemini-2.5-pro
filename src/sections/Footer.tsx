...
const Footer: React.FC = () => {
  return (
    <footer className="bg-[#0a0a0a] border-t border-white/10 py-16 px-4">
      <div className="container mx-auto max-w-6xl">
        <div className="grid md:grid-cols-4 gap-12">
          <div className="md:col-span-2">
            <a href="#hero" className="text-2xl font-bold text-primary-yellow tracking-tight">
              Travel<span className="text-white">.</span>
            </a>
            <p className="mt-4 text-white/70 max-w-sm">
              We craft unforgettable journeys to the world’s most beautiful places. Your next adventure is one click away.
            </p>
          </div>

          <div>
            <h4 className="text-sm font-semibold text-primary-yellow uppercase tracking-wider mb-4">
              Quick Links
            </h4>
            <nav className="space-y-3">
              {footerLinks.map((link) => (
                <a
                  key={link.label}
                  href={link.href}
                  className="block text-white/70 hover:text-primary-yellow text-sm transition-colors"
                >
                  {link.label}
                </a>
              ))}
            ...
            <div className="flex gap-4">
              {socialLinks.map((s) => (
                <a
                  ...
                  className="text-white/60 hover:text-primary-yellow transition-colors"
                  aria-label={s.label}
                >
